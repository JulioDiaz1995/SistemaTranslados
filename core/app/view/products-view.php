<div class="row">
	<div class="col-md-12">
		...
		if (count($products) > 0) {
			...
			if ($page == 1) {
				$curr_products = ProductData::TraerProductoPorUbicacion($products[0]->id, $limit, $ubicacion);
			} else {
				$curr_products = ProductData::TraerProductoPorUbicacion($products[($page - 1) * $limit]->id, $limit, $ubicacion);
			}
		...
		?>
			...
			<select name="bodega" id="bodega" onchange='CambioBodega()'>
				<?php
				$ubi = ProductData::TraerUbicacion();

				?>
				<option value="">Seleccione Taller:</option>
				<?php foreach ($ubi as $product) : ?>
					<option><?php echo $product->price_in; ?></option>
				<?php endforeach; ?>
			</select>
			...
			<div class="btn-group pull-right">
				...
				for ($i = 0; $i < $npaginas; $i++) {
					echo "<a href='index.php?view=products&limit=$limit&page=" . ($i + 1) . "&ubicacion=$ubicacion' class='btn btn-default btn-sm'>" . ($i + 1) . "</a> ";
				}
				?>
			</div>
	...
	<script>
		function CambioBodega() {
			window.location = 'index.php?view=products&ubicacion=' + document.getElementById('bodega').value;
		}
	</script>
